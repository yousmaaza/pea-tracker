# PEA Tracker - Suivi Intelligent de Portefeuille

## Vue d'ensemble du projet

PEA Tracker est une plateforme intelligente de gestion de portefeuille PEA (Plan d'Épargne en Actions) utilisant des agents IA autonomes Claude pour optimiser les décisions d'investissement. Le système automatise la surveillance des marchés et l'analyse de portefeuille pour aider les investisseurs particuliers à prendre des décisions éclairées sur les titres éligibles PEA.

## Vision

Créer une solution légère et intelligente basée sur le **Model Context Protocol (MCP)** et **Claude Code**, minimisant la complexité technique tout en offrant des analyses financières avancées grâce à l'IA.

## Objectifs principaux

- **Automatiser** la surveillance des marchés financiers européens
- **Analyser** automatiquement les positions du portefeuille
- **Alerter** sur les opportunités d'achat/vente avec scoring de fiabilité
- **Recommander** des ajustements stratégiques basés sur l'historique
- **Rapporter** mensuellement sur la performance et l'allocation

## Utilisateur cible

Investisseur particulier détenteur d'un PEA souhaitant optimiser sa gestion de portefeuille grâce à l'intelligence artificielle, sans avoir besoin de surveiller constamment les marchés.

## Périmètre géographique

Le système couvre les marchés européens éligibles au PEA :
- France (Euronext Paris)
- Allemagne (Xetra)
- Italie (Borsa Italiana)
- Espagne (BME)
- Pays-Bas (Euronext Amsterdam)
- Belgique (Euronext Brussels)
- Portugal (Euronext Lisbon)
- ETF éligibles PEA (structure européenne)

## Architecture technique

### Philosophie

Architecture **MCP-native** utilisant Claude Code comme environnement d'exécution. Les agents Claude accèdent directement aux sources de données via des serveurs MCP standardisés, éliminant complètement le besoin d'infrastructure intermédiaire (n8n, serveurs, etc.).

### Flux de données simplifié

```
Boursorama → Export Excel → Google Drive ←→ Claude Agent (MCP) → Gmail
```

### Stack technique actuelle

**Environnement d'exécution**
- **Claude Code** : CLI tool pour exécuter les agents IA
- **Agents personnalisés** : Définis dans `.claude/agents/`
- **Exécution** : Manuelle via CLI ou automatisée via cron/scheduler système

**Serveurs MCP configurés**
- `@modelcontextprotocol/server-google-drive` : Accès Google Drive/Sheets
- `@modelcontextprotocol/server-gmail` : Envoi d'emails
- `mcp-server-yfinance` : Données boursières Yahoo Finance (temps réel)
- `@modelcontextprotocol/server-github` : Gestion du code source
- `@modelcontextprotocol/server-filesystem` : Accès au système de fichiers local

**Stockage et données**
- Google Drive : Stockage des exports Boursorama, historiques et rapports
- Structure de dossiers Google Drive :
    ```
    PEA-Tracker/
    ├── Imports/                           # Boursorama exports (transaction history)
    │   └── export-positions-comptables-DD-MM-YYYY_HH-MM-SS.csv
    ├── Reports/                           # Reports generated by agents
    │   ├── monthly/                      # Monthly Portfolio Advisor reports
    │   └── signals/                      # Market Watcher alerts
    │       └── signal_TICKER_YYYYMMDD_HHMM.md
    └── PEA_Watchlist_Indicateurs.xlsx    # Main watchlist with technical indicators
    ```

**Données financières**
- Yahoo Finance via MCP : Données boursières en temps réel (gratuit)
- Indicateurs techniques calculés par l'agent (RSI, MACD, MA)

**Notifications**
- Gmail via MCP : Envoi automatique des alertes et rapports

### Documentation technique

- Architecture MCP : [docs/architecture/mcp-integration.md](docs/architecture/mcp-integration.md)
- Yahoo Finance MCP : [docs/mcp/yahoo-finance-integration.md](docs/mcp/yahoo-finance-integration.md)

## Les agents IA

### Agent 1 : Market Watcher PEA ✅ IMPLÉMENTÉ

**Fichier** : `.claude/agents/market-watcher-pea.md`

**Mission** : Surveillance quotidienne des marchés et génération d'alertes d'opportunités (achat/vente) sur les titres éligibles PEA.

**Responsabilités** :
- Récupérer la watchlist depuis Google Drive (`PEA_Watchlist_Indicateurs.xlsx`)
- Collecter les données de marché via Yahoo Finance MCP
- Calculer les indicateurs techniques (RSI, MACD, MA20/50/200)
- Détecter les signaux d'achat/vente/surveillance
- Scorer la fiabilité de chaque signal (0-100)
- Générer des rapports Markdown détaillés
- Sauvegarder les rapports dans Google Drive
- Envoyer des alertes par email (score ≥ 60)

**Workflow complet** :
1. Récupération du fichier Excel depuis Google Drive
2. Parsing des feuilles "Watchlist", "Indicateurs", "Positions"
3. Collecte des données de marché (prix, volume, historique)
4. Calcul des indicateurs techniques pour chaque ticker actif
5. Génération des signaux avec scoring de confiance
6. Mise à jour du fichier Excel avec nouveaux indicateurs
7. Création de rapports Markdown pour signaux ≥ 60
8. Upload des rapports dans Google Drive
9. Envoi d'emails d'alerte via Gmail

**Indicateurs techniques** :
- RSI (14 périodes) : Détection de surachat/survente
- MACD (12, 26, 9) : Identification des tendances
- Moyennes mobiles (MA20, MA50, MA200) : Support/résistance
- Volume ratio : Confirmation des mouvements

**Signaux générés** :
- 🟢 **BUY** : RSI < 30, volume élevé, MACD haussier, prix > MA200
- 🔴 **SELL** : RSI > 70, divergence RSI, MACD baissier, cassure MA200
- 🟡 **WATCH** : RSI 30-40 ou 60-70, consolidation, signaux mixtes

**Fréquence recommandée** : Quotidienne à 8h (avant ouverture des marchés)

**Invocation** :
```bash
# Via Claude Code
claude-code agent run market-watcher-pea

# Ou en mode interactif
claude-code
> invoke @market-watcher-pea
```

**Spécifications complètes** : [docs/agents/market-watcher-spec.md](docs/agents/market-watcher-spec.md)

### Agent 2 : Portfolio Advisor 🔜 À DÉVELOPPER

**Mission** : Analyse mensuelle du portefeuille, suivi de la performance et recommandations stratégiques.

**Responsabilités prévues** :
- Calculer la performance globale et par ligne
- Analyser l'allocation sectorielle et géographique
- Évaluer le niveau de diversification
- Générer des rapports mensuels détaillés
- Proposer des recommandations de rééquilibrage
- Suivre les objectifs d'investissement

**Fréquence recommandée** : Mensuelle (1er du mois) + à la demande

**Statut** : Spécifications définies, implémentation à venir

**Spécifications complètes** : [docs/agents/portfolio-advisor-spec.md](docs/agents/portfolio-advisor-spec.md)

## Structure du projet

```
claude-project/
├── .claude/
│   ├── agents/
│   │   └── market-watcher-pea.md         # ✅ Agent Market Watcher implémenté
│   └── settings.local.json                # Configuration MCP locale
│
├── docs/
│   ├── agents/
│   │   ├── market-watcher-spec.md         # Spécifications Market Watcher
│   │   └── portfolio-advisor-spec.md      # Spécifications Portfolio Advisor
│   ├── architecture/
│   │   └── mcp-integration.md             # Documentation architecture MCP
│   ├── mcp/
│   │   └── yahoo-finance-integration.md   # Guide Yahoo Finance MCP
│   ├── ARCHITECTURE_DECISION.md           # Décisions architecture (n8n → MCP)
│   └── SETUP_GUIDE.md                     # Guide d'installation
│
├── prompts/
│   ├── market-analysis.md                 # Prompt template Market Watcher
│   ├── portfolio-review.md                # Prompt template Portfolio Advisor
│   └── prompt/                            # Anciens prompts de test
│
├── scripts/                               # (Vide - plus nécessaire avec MCP)
│
├── CLAUDE.md                              # 📄 Ce fichier
├── README.md                              # Documentation utilisateur
└── TODO.md                                # Liste des tâches à venir
```

## Prérequis et configuration

### Prérequis

| Élément | Description | Coût |
|---------|-------------|------|
| Claude Code | CLI tool Anthropic | Gratuit |
| Clé API Claude | API Anthropic | 5-20€/mois |
| Google Workspace | Drive + Gmail | Gratuit |
| MCP Servers | Serveurs MCP standards | Gratuit |
| Yahoo Finance | Via MCP | Gratuit |

**Total estimé** : **5-20€/mois** (uniquement l'API Claude)

### Installation

1. **Installer Claude Code** :
```bash
npm install -g @anthropic-ai/claude-code
```

2. **Configurer l'API Claude** :
```bash
export ANTHROPIC_API_KEY="your-api-key"
```

3. **Configurer les serveurs MCP** :
   - Éditer `.claude/settings.local.json`
   - Ajouter les serveurs nécessaires (Google Drive, Gmail, Yahoo Finance)

4. **Configurer Google Drive** :
   - Créer la structure de dossiers `PEA-Tracker/`
   - Créer le fichier `PEA_Watchlist_Indicateurs.xlsx`
   - Configurer les authentifications OAuth

5. **Configuration Gmail** :
   - Générer un mot de passe d'application
   - Configurer le serveur MCP Gmail

### Documentation d'installation complète

Voir [docs/SETUP_GUIDE.md](docs/SETUP_GUIDE.md)

## Utilisation

### 1. Market Watcher - Surveillance quotidienne

**Lancement manuel** :
```bash
claude-code agent run market-watcher-pea
```

**Automatisation via cron** (recommandé) :
```bash
# Éditer crontab
crontab -e

# Ajouter cette ligne pour exécution quotidienne à 8h
0 8 * * 1-5 cd /path/to/claude-project && claude-code agent run market-watcher-pea
```

**Ce que fait l'agent** :
1. ✅ Récupère la watchlist depuis Google Drive
2. ✅ Analyse les titres actifs
3. ✅ Calcule les indicateurs techniques
4. ✅ Génère les signaux d'achat/vente
5. ✅ Sauvegarde les rapports dans Google Drive
6. ✅ Envoie les alertes par email (score ≥ 60)

### 2. Portfolio Advisor - Analyse mensuelle (à venir)

**Prévu** : Analyse automatique le 1er de chaque mois

### 3. Export Boursorama

**Procédure** :
1. Se connecter à Boursorama
2. PEA → Télécharger les positions comptables (format CSV)
3. Uploader le fichier dans Google Drive/PEA-Tracker/Imports/
4. L'agent Portfolio Advisor utilisera ces données pour l'analyse

## Avantages de l'architecture MCP

### ✅ Simplicité
- Pas de serveur à maintenir
- Pas de base de données à gérer
- Pas d'orchestrateur complexe (n8n)
- Configuration minimale

### ✅ Coûts réduits
- Infrastructure : 0€ (pas de serveur)
- API Claude : 5-20€/mois uniquement
- Total : **5-20€/mois** (vs 25-60€/mois avec n8n)

### ✅ Fiabilité
- Moins de points de défaillance
- Agents autonomes et résilients
- Gestion d'erreurs intégrée

### ✅ Évolutivité
- Ajout facile de nouveaux serveurs MCP
- Création simple de nouveaux agents
- Réutilisation de serveurs MCP standards

## Sécurité

- ✅ Clés API stockées dans variables d'environnement
- ✅ Authentification OAuth pour Google services
- ✅ Pas de stockage d'identifiants broker
- ✅ Accès restreint aux dossiers Google Drive
- ✅ Communications chiffrées (HTTPS)
- ⚠️ Ne jamais commiter les fichiers de configuration avec secrets

## Roadmap

### Phase 1 : Infrastructure MCP ✅ TERMINÉE
- [x] Configuration Claude Code
- [x] Installation serveurs MCP
- [x] Authentification Google Drive/Gmail
- [x] Structure Google Drive créée
- [x] Agent Market Watcher implémenté

### Phase 2 : Portfolio Advisor 🔄 EN COURS
- [ ] Implémenter l'agent Portfolio Advisor
- [ ] Parser les exports Boursorama (CSV)
- [ ] Calculer les métriques de performance
- [ ] Générer les rapports mensuels
- [ ] Automatiser l'envoi des rapports

### Phase 3 : Optimisations 📋 PLANIFIÉE
- [ ] Backtesting des signaux Market Watcher
- [ ] Profil de risque personnalisé
- [ ] Intégration actualités financières (RSS/API)
- [ ] Dashboard web simple (optionnel)
- [ ] Alertes Telegram/SMS (optionnel)

### Phase 4 : Améliorations avancées 🔮 FUTUR
- [ ] Machine Learning pour scoring amélioré
- [ ] Analyse sentiment market (NLP)
- [ ] Intégration données fondamentales
- [ ] Support multi-portefeuilles

## Points d'attention
- Si besoin d'utiliser du python il faudra s'assurer que le profile d'environnement 

### Réglementaire
⚠️ **Important** : Ce système ne constitue pas du conseil en investissement au sens AMF. Tous les rapports et alertes incluent un disclaimer approprié. Les décisions d'investissement restent sous la responsabilité de l'utilisateur.

### Limitations techniques
- Dépendance aux APIs externes (Yahoo Finance, Google)
- Rate limiting possible sur les APIs gratuites
- Données de marché avec léger délai (non temps réel strict)
- Nécessite une connexion internet stable

### Bonnes pratiques
- Avant d'utiliser les agents tu peux creer un environnement virtuel python et utiliser la commande `/pip-mode standard` pour installer les dépendances python sans conflit.
- utilise du Python 3.11 ou supérieur
- Toujours vérifier les signaux avant d'agir
- Utiliser les stop-loss recommandés
- Ne jamais investir plus que ce qu'on peut perdre
- Diversifier le portefeuille
- Suivre une stratégie d'investissement cohérente

## Support et ressources

### Documentation
- [Architecture MCP](docs/architecture/mcp-integration.md)
- [Guide d'installation](docs/SETUP_GUIDE.md)
- [Spécifications agents](docs/agents/)

### Ressources externes
- [Claude Code Documentation](https://docs.anthropic.com/claude-code)
- [Model Context Protocol](https://modelcontextprotocol.io/)
- [Yahoo Finance API](https://www.yahoofinanceapi.com/)
- [Éligibilité PEA](https://www.service-public.fr/particuliers/vosdroits/F2385)

## Glossary

- **PEA** : Plan d'Épargne en Actions - Enveloppe fiscale française
- **MCP** : Model Context Protocol - Standard pour connecter les LLM aux sources de données
- **RSI** : Relative Strength Index - Indicateur de momentum
- **MACD** : Moving Average Convergence Divergence - Indicateur de tendance
- **MA** : Moving Average - Moyenne mobile
- **Claude Code** : CLI tool pour exécuter des agents Claude
- **Boursorama** : Courtier en ligne français
- **ETF** : Exchange Traded Fund - Fonds négocié en bourse

## Disclaimer

⚠️ **AVERTISSEMENT IMPORTANT**

Ce projet est un **outil d'aide à la décision** utilisant l'analyse technique automatisée. Il **NE CONSTITUE EN AUCUN CAS UN CONSEIL EN INVESTISSEMENT** au sens de l'AMF (Autorité des Marchés Financiers).

**Responsabilités** :
- Toutes les décisions d'investissement restent sous votre **entière responsabilité**
- Les performances passées ne préjugent **pas** des performances futures
- Les marchés financiers comportent des **risques de perte en capital**
- Vous devez toujours effectuer vos **propres recherches**
- Consultez un conseiller financier agréé si nécessaire

**Limites de l'outil** :
- Les analyses sont basées uniquement sur des indicateurs techniques
- Les données peuvent contenir des erreurs ou retards
- L'IA peut générer des recommandations erronées
- Aucune garantie de rentabilité n'est fournie

**Utilisation à vos risques et périls.**

## Licence

[À définir selon vos besoins]

---

**Version** : 2.0.0
**Dernière mise à jour** : 2026-01-07
**Statut** : Phase 1 terminée, Agent Market Watcher opérationnel
**Architecture** : MCP-native avec Claude Code

Créé avec ❤️ et Claude Code
